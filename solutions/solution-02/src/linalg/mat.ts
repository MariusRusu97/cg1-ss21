 ...
 */
import {IVector, Vector} from './vec';
import {approxEqual} from './utils';

/**
 * IMatrix is an interface that defines matrix operations.
 * A Matrix implementation can have m x n dimentions.
 */
export interface IMatrix {
  set(i: number, j: number, value: number): void;
  get(i: number, j: number): number;
  eq(m: IMatrix): boolean;
  add(m: IMatrix): IMatrix;
  sub(m: IMatrix): IMatrix;
  mul(m: IMatrix | IVector): IMatrix | IVector;
  T(): IMatrix;
  inv(): IMatrix;
  det(): number;
}
...
export class Matrix implements IMatrix {
   ...
   */
  mul(m: Matrix | Vector): Matrix | IVector {
